...
export async function registerShop(
  ...
  const user = await User.create({ name: 'Owner', email, passwordHash: await hashPassword(password), role: Role.SHOP_ADMIN });
  const shop = await provisionShop(undefined, user, overrides.shopName ?? 'Test Dairy', { status: overrides.status ?? ShopStatus.PENDING, seedCategories: false });
  user.shopId = shop._id;
  await user.save();

  const res = await request(app).post('/api/v1/auth/login').send({ email, password });
  if (res.status !== 200) throw new Error(`registerShop login failed: ${res.status} ${JSON.stringify(res.body)}`);
  return { token: res.body.data.accessToken, userId: user._id.toString(), shopId: shop._id.toString(), email };
}
...
export async function createSuperAdmin(app: Express): Promise<TestActor> {
  ...
  await User.create({ name: 'Super', email, passwordHash: await hashPassword(password), role: Role.SUPER_ADMIN });
  const res = await request(app).post('/api/v1/auth/login').send({ email, password });
